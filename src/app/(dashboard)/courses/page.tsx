...
const getCourseStyle = (code: string | null | undefined) => {
  ...
  if (upperCode.startsWith('CS') || upperCode.startsWith('CST') || upperCode.startsWith('COMP')) {
    return {
      color: 'bg-blue-600',
      bg: 'bg-blue-600/10',
      text: 'text-blue-400',
...
const CourseCard = ({ course, isEnrolled, onEnroll }: { course: any; isEnrolled: boolean; onEnroll: () => void }) => {
  ...
  const room = course.schedule?.includes('Room') ? course.schedule.split('Room')[1]?.trim() : 'TBA';

  return (
    <div className={`bg-[#121212] border border-gray-800 rounded-2xl p-5 hover:border-gray-600 transition group relative overflow-hidden ${isEnrolled ? 'opacity-75' : ''}`}>
      
      {/* Top: Course Code Badge + Status */}
      <div className="flex justify-between items-start mb-4">
        {/* Dynamic colored course code box */}
        <div className={`w-16 h-16 rounded-xl ${style.color} flex items-center justify-center text-white font-bold shadow-lg flex-shrink-0`}>
      ...
      <div className="pt-4 border-t border-gray-800 flex justify-between items-center">
        <div className="flex flex-col">
          <span className="text-xs text-gray-500">Section {course.section}</span>
          <span className="text-xs text-gray-500">{course.semester}</span>
        </div>
        
        {isEnrolled ? (
          <span className="text-sm text-indigo-400 font-semibold">✓ Enrolled</span>
      ...
      <main className="relative z-10 w-full p-6 md:p-10">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
            <div>
              <h1 className="text-4xl md:text-5xl font-black text-white mb-2">Course Catalog</h1>
              <p className="text-gray-400 text-lg">Search and enroll in courses for {new Date().getFullYear()}</p>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                {enrolledCourses.map((course) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div className={`w-16 h-16 rounded-xl ${style.color} flex items-center justify-center text-white font-bold shadow-lg`}>
                          <div className="text-center leading-tight">
                            <span className="text-xs block">{department}</span>
                            <span className="text-sm">{number}</span>
                          </div>
                      ...
                      <div className="pt-4 border-t border-gray-800">
                        <GlowingButton variant="outline" href={`/matches?courseId=${course.id}`} className="w-full text-sm py-2">
                          View Matches
                        </GlowingButton>
                      </div>
                    </div>
          ...
          {enrolledCourses.length === 0 && courses.length === 0 && !loading && (
            <div className="bg-[#121212] border border-gray-800 rounded-3xl p-12 text-center">
              ...
              <p className="text-gray-400 mb-6">Search and enroll in courses to start finding study partners</p>
            </div>
          )}
        </div>
      </main>
