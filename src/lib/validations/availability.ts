...
export const availabilitySlotWithValidationSchema = availabilitySlotSchema.refine(
  (data) => {
...
export const createAvailabilitySchema = z.object({
  slots: z.array(availabilitySlotWithValidationSchema).min(1, 'At least one time slot is required'),
});
...
export const updateAvailabilitySchema = z.object({
  ...
  {
    message: 'Both start time and end time must be provided when updating times',
  }
).refine(
  (data) => {
