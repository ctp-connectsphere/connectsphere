...
export async function getRedisInfo() {
  try {
    ...
    throw new RedisError('Failed to get Redis information', 'INFO_ERROR', error)
  }
}
  ...
  const connectWithRetry = async () => {
    try {
      ...
      if (retryCount < maxRetries) {
        ...
        console.error('❌ Max Redis retry attempts reached. Please check your Redis connection.')
        throw new RedisError('Max retry attempts reached', 'MAX_RETRY_ERROR', error)
      }
    }
  }
