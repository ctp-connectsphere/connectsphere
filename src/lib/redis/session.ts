...
export class SessionService {
  ...
  // Session management
  static async createSession(sessionToken: string, sessionData: any, ttl = this.TTL.SESSION) {
    try {
  ...
  static async updateSession(sessionToken: string, sessionData: any, ttl = this.TTL.SESSION) {
    try {
      const key = `${this.KEY_PATTERNS.SESSION}${sessionToken}`
      await redis.setex(key, ttl, JSON.stringify(sessionData))
      return true
  ...
  static async createRefreshToken(userId: string, refreshToken: string, deviceInfo: any, ttl = this.TTL.REFRESH_TOKEN) {
    try {
  ...
  static async createDeviceSession(userId: string, deviceId: string, deviceInfo: any, ttl = this.TTL.DEVICE_SESSION) {
    try {
  ...
  static async updateDeviceSession(userId: string, deviceId: string, deviceInfo: any, ttl = this.TTL.DEVICE_SESSION) {
    try {
      const key = `${this.KEY_PATTERNS.DEVICE_SESSION}${userId}:${deviceId}`
