...
export class SessionValidation {
    ...
    static async validateSession(): Promise<SessionValidationResult> {
        try {
            ...
            if (session.user.id) {
                ...
                const now = Date.now()
                const shouldRefresh = redisSession.exp && (redisSession.exp * 1000 - now) < this.SESSION_REFRESH_THRESHOLD
     ...
     */
    static async refreshSession(sessionToken: string, sessionData: any): Promise<boolean> {
        try {
     ...
     */
    static async validateRequestSession(request: NextRequest): Promise<SessionValidationResult> {
        try {
            ...
            if (sessionToken) {
                const redisSession = await SessionService.getSession(sessionToken.value)
                if (!redisSession) {
                    return {
                        isValid: false,
            ...
            return {
                isValid: false,
                error: error instanceof Error ? error.message : 'Request session validation failed'
            }
        }
    }
