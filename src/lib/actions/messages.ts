...
export async function getMessages(connectionId: string, limit = 50) {
    try {
        ...
        const otherUser = connection.requesterId === userId ? connection.target : connection.requester;
        const matchContext = connection.course
            ? { type: 'course', name: connection.course.name, code: connection.course.code }
            : connection.topic
                ? { type: 'topic', name: connection.topic.name, category: connection.topic.category }
                : null;
    ...
    try {
        ...
        const conversations = connections.map(conn => {
            ...
            const matchContext = conn.course
                ...
                : conn.topic
                    ? { type: 'topic', name: conn.topic.name, category: conn.topic.category }
                    : null;
