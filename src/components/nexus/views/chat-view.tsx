...
import { 
  Send, Paperclip, Code, Calendar, MoreVertical, 
  Phone, Video, Search, Hash, ChevronLeft, User, X,
  Copy, Check
} from 'lucide-react';
import { useSession } from 'next-auth/react';
import { getMessages, sendMessage, getConversations } from '@/lib/actions/messages';
import { useParams, useRouter } from 'next/navigation';

// Code Block Component with Syntax Highlighting
const CodeBlock = ({ code, language = 'text', onCopy }: { code: string; language?: string; onCopy?: () => void }) => {
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 w-72 text-left">
      <div className="flex items-center gap-2 mb-3 text-indigo-400">
        <Calendar size={16} />
        <span className="text-xs font-bold uppercase tracking-wide">Study Session</span>
      </div>
  ...
  const loadConversations = async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        if (connectionId) {
          const active = result.data.conversations.find((c: any) => c.id === connectionId);
      ...
      <div className="flex h-full bg-[#0a0a0a] text-white">
        ...
        <div className="w-80 border-r border-gray-800 flex flex-col bg-[#111]">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            <div className="text-xs font-bold text-gray-500 px-2 uppercase tracking-wider mb-2 mt-2">Direct Messages</div>
            
            {conversations.length === 0 ? (
              <div className="text-center py-12 text-gray-500">
                <p className="text-sm">No conversations yet</p>
                <p className="text-xs mt-2">Start by connecting with someone!</p>
              </div>
            ) : (
                  ...
                  >
                    <div className="relative">
                      <div 
                        ...
                        style={{
                          backgroundImage: otherUser.profileImageUrl ? `url(${otherUser.profileImageUrl})` : undefined,
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-baseline">
                        <h3 className="font-medium truncate text-sm">{otherUser.firstName} {otherUser.lastName?.charAt(0)}.</h3>
                        <span className="text-xs text-gray-400 ml-2">{formatTime(conv.updatedAt)}</span>
                      </div>
                      <p className="text-xs text-gray-400 truncate mt-0.5">
                        {conv.lastMessage?.isMe ? 'You: ' : ''}
                        {conv.lastMessage?.messageType === 'code' ? '📝 Code snippet' : 
                         conv.lastMessage?.messageType === 'invite' ? '📅 Study invite' :
                ...
                >
                  <div className="relative">
                    <div 
                      className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center font-bold text-sm"
                      style={{
                        backgroundImage: other.profileImageUrl ? `url(${other.profileImageUrl})` : undefined,
                        backgroundSize: 'cover',
                      }}
                    >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-baseline">
                      <h3 className="font-medium truncate text-sm">{other.firstName} {other.lastName?.charAt(0)}.</h3>
                      <span className="text-xs text-gray-400 ml-2">{formatTime(conv.updatedAt)}</span>
                    </div>
                    <p className="text-xs text-gray-400 truncate mt-0.5">
                      {conv.lastMessage?.isMe ? 'You: ' : ''}
      ...
      <div className="flex-1 flex flex-col relative bg-[#0a0a0a]">
        ...
        <div className="h-16 border-b border-gray-800 flex items-center justify-between px-6 bg-[#0a0a0a]/80 backdrop-blur-md sticky top-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-sm">{otherUser?.firstName} {otherUser?.lastName?.charAt(0)}.</h2>
              {matchContext && (
                <p className="text-xs text-indigo-400 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span>
                  Matched on {matchContext.type === 'course' ? matchContext.code || matchContext.name : matchContext.name}
                </p>
              )}
            </div>
          </div>
          ...
          {messages.map((msg, idx) => {
            ...
            return (
              <div key={msg.id} className={`flex ${msg.isMe ? 'justify-end' : 'justify-start'}`}>
                {!msg.isMe && (
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-blue-500 flex-shrink-0 mr-3 mt-1 flex items-center justify-center text-xs font-bold"
                    style={{
                      backgroundImage: msg.sender?.profileImageUrl ? `url(${msg.sender.profileImageUrl})` : undefined,
                      backgroundSize: 'cover',
                    }}
                  >
                    {!msg.sender?.profileImageUrl && `${msg.sender?.firstName?.charAt(0) || ''}${msg.sender?.lastName?.charAt(0) || ''}`}
                  </div>
                )}
                ...
                <div className={`max-w-[70%] space-y-1 ${msg.isMe ? 'items-end flex flex-col' : ''}`}>
                  ...
                  {msg.messageType === 'invite' && (
                    <InviteCard
                      title="Study Session"
                      date={msg.content.includes('Tomorrow') ? 'Tomorrow, 2:00 PM' : 'TBA'}
                      location={msg.content.includes('Library') ? 'Library Room 304' : undefined}
                      status="pending"
                      onAccept={() => {
                        // Handle accept
      ...
      {showDetails && otherUser && (
        <>
          ...
          <div className="hidden md:flex w-80 border-l border-gray-800 bg-[#111] flex-col p-6">
            ...
            {/* Profile Summary */}
            <div className="mb-6">
              <div className="w-20 h-20 rounded-full bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center text-2xl font-bold mb-4 mx-auto"
                style={{
                  backgroundImage: otherUser.profileImageUrl ? `url(${otherUser.profileImageUrl})` : undefined,
                  backgroundSize: 'cover',
                }}
              ...
              {matchContext && (
                <p className="text-center text-sm text-gray-400">
                  {matchContext.type === 'course' ? matchContext.code : matchContext.name}
                </p>
              )}
            </div>
            ...
            {matchContext && (
              <div className="mb-6">
                <h5 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Common Interests</h5>
                <div className="space-y-2">
                  <div className="px-3 py-2 bg-gray-800/50 rounded-lg">
                    <span className="text-sm text-white">{matchContext.name}</span>
                  </div>
                </div>
              </div>
            )}
