...
import { Calendar, Clock, Send, MoreHorizontal, Zap, TrendingUp, Users, BookOpen, Brain, Plus, ArrowRight, Activity, Target } from 'lucide-react';
import { Badge } from '../ui/badge';
import { GlowingButton } from '../ui/glowing-button';
import { useState, useEffect } from 'react';
import { useSession } from 'next-auth/react';
import { getDashboardStats, getRecommendedPeers, getActiveGroups, getUpcomingSessions } from '@/lib/actions/dashboard';
...
export const DashboardView = ({ userName = 'User' }: DashboardViewProps) => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (sessionsResult.success && sessionsResult.data && sessionsResult.data.sessions.length > 0) {
        ...
        if (topicsResult.success && topicsResult.data && topicsResult.data.userTopics.length > 0) {
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 relative z-10">
        <div>
          <h1 className="text-4xl md:text-5xl font-black text-white mb-2">
            {getGreeting()}, <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-pink-400">{userFirstName}</span>
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-7xl mx-auto relative z-10">
            ...
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {upcomingSession ? (
                    <h2 className="text-2xl md:text-3xl font-black text-white mb-1">{upcomingSession.topic}</h2>
                  ) : (
                    <h2 className="text-2xl md:text-3xl font-black text-white mb-1">No upcoming sessions</h2>
                  )}
                </div>
              </div>
            </div>
              ...
              <>
                {upcomingSession.isSuggestion ? (
                  <>
                    ...
                    <Link href="/matches">
                      <GlowingButton variant="secondary" className="bg-white text-black hover:bg-gray-100">
                        Find Study Partner
                    ...
                    <div className="space-y-3 mb-6">
                      ...
                      {upcomingSession.courseCode && (
                        <div className="flex items-center gap-2">
                          <Badge color="indigo">{upcomingSession.courseCode}</Badge>
                          <span className="text-gray-400 text-sm">{upcomingSession.course}</span>
                        </div>
                      )}
                      {upcomingSession.organizer && (
                        <div className="flex items-center gap-2 text-gray-400 text-sm">
                          <span>Organized by {upcomingSession.organizer.name}</span>
                        </div>
                      )}
                    </div>
            ...
            <div>
              ...
              <div className="h-2.5 bg-gray-800 rounded-full overflow-hidden">
                <div 
                  ...
                  style={{ width: `${Math.min((stats.topics / 10) * 100, 100)}%` }}
                ></div>
              </div>
            </div>
            <div>
              <div className="flex justify-between text-xs text-gray-400 mb-2">
                ...
                <span className="font-semibold text-white">{stats.courses}</span>
              </div>
              <div className="h-2.5 bg-gray-800 rounded-full overflow-hidden">
                <div 
                  className="h-full bg-gradient-to-r from-blue-500 to-cyan-500 rounded-full transition-all"
                  style={{ width: `${Math.min((stats.courses / 5) * 100, 100)}%` }}
                ></div>
              </div>
            </div>
            <div>
              <div className="flex justify-between text-xs text-gray-400 mb-2">
                ...
                <span className="font-semibold text-white">{stats.connections}</span>
              </div>
              <div className="h-2.5 bg-gray-800 rounded-full overflow-hidden">
                <div 
                  className="h-full bg-gradient-to-r from-green-500 to-emerald-400 rounded-full transition-all"
                  style={{ width: `${Math.min((stats.connections / 10) * 100, 100)}%` }}
                ></div>
              </div>
            </div>
          </div>
        ...
        <div className="md:row-span-2 bg-[#121212] border border-gray-800/50 rounded-3xl p-6 hover:border-gray-700/50 transition-all">
          <div className="flex justify-between items-center mb-6">
            ...
            <Link href="/matches" className="text-xs text-indigo-400 hover:text-indigo-300 font-medium">
              View all
            </Link>
          </div>
          ...
          {recommendedPeers.length > 0 ? (
                ...
                <Link key={peer.id} href="/matches">
                  <div className="flex items-center gap-3 p-3 hover:bg-gray-800/50 rounded-xl transition cursor-pointer group">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="text-sm font-bold text-white truncate">{peer.firstName} {peer.lastName?.charAt(0)}.</h4>
                      <p className="text-xs text-gray-400 truncate">{peer.matchScore}% Match • {peer.commonTopics[0] || 'Topic'}</p>
                    </div>
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-gray-400 text-sm mb-4">No recommendations yet</p>
              <Link href="/topics">
                <GlowingButton variant="secondary" className="text-sm">Add Topics</GlowingButton>
              </Link>
            </div>
          )}
        </div>
        ...
        {activeGroups.length > 0 ? (
          <div className="md:col-span-2 bg-[#121212] border border-gray-800/50 rounded-3xl p-6 hover:border-gray-700/50 transition-all">
            <div className="flex justify-between items-center mb-6">
              ...
              <Link href="/groups" className="text-xs text-indigo-400 hover:text-indigo-300 font-medium">
                View all
              </Link>
            </div>
                ...
                <Link key={group.id} href={`/chat/${group.id}`}>
                  <div className="bg-gray-800/30 p-5 rounded-2xl border border-gray-700/50 hover:border-indigo-500/50 transition-all group">
                    <div className="flex justify-between items-start mb-3">
                      <Badge color={group.categoryBadge as any}>{group.categoryLabel || group.category}</Badge>
                      <span className="text-xs text-gray-500 font-semibold">{group.memberCount}/2</span>
                    </div>
                    <h4 className="text-lg font-black text-white group-hover:text-indigo-300 transition-colors mb-2">{group.name}</h4>
                    <p className="text-xs text-gray-400">Last active: {group.lastActive}</p>
                  </div>
                </Link>
              ))}
            </div>
          ...
          <div className="md:col-span-2 bg-gradient-to-br from-[#121212] to-[#1a1a1a] border border-gray-800/50 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between hover:border-gray-700/50 transition-all">
            <div className="flex-1">
              <h3 className="text-xl font-black text-white mb-2">No active groups yet?</h3>
              <p className="text-gray-400 text-sm mb-4">
                Join a study squad to boost your productivity and connect with peers.
              </p>
              <div className="flex flex-wrap gap-2">
                <span className="px-3 py-1.5 bg-gray-800/50 rounded-lg text-xs text-gray-300 font-medium border border-gray-700/50">#LeetCode</span>
                <span className="px-3 py-1.5 bg-gray-800/50 rounded-lg text-xs text-gray-300 font-medium border border-gray-700/50">#WebDev</span>
                <span className="px-3 py-1.5 bg-gray-800/50 rounded-lg text-xs text-gray-300 font-medium border border-gray-700/50">#Python</span>
                <span className="px-3 py-1.5 bg-gray-800/50 rounded-lg text-xs text-gray-300 font-medium border border-gray-700/50">#DataScience</span>
              </div>
            </div>
            <div className="mt-6 md:mt-0 md:ml-6">
              <Link href="/groups">
                <GlowingButton variant="secondary" className="bg-gray-800 border border-gray-700 text-white hover:bg-gray-700">
                  Browse Groups
                  <ArrowRight size={18} className="ml-2" />
                </GlowingButton>
              </Link>
