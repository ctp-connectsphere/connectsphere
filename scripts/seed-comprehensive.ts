...
async function seedComprehensive() {
  ...
  for (const user of allUsers) {
    ...
    const userUniversity = createdUniversities.find(
  ...
  for (let i = 0; i < Math.min(20, allUsers.length / 2); i++) {
    ...
    // Get a random course that both users might share (use first available course)
    const availableCourses = await prisma.course.findMany({ take: 10 });
    if (availableCourses.length === 0) continue; // Skip if no courses
    const randomCourse =
